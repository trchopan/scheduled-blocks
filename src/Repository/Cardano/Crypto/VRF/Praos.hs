...
module Repository.Cardano.Crypto.VRF.Praos
  (
  -- * VRFAlgorithm API
    PraosVRF
...
genSeed = do
  seed <- mkSeed
  withForeignPtr (unSeed seed)
    $ \ptr -> randombytes_buf ptr crypto_vrf_seedbytes
  return seed

copyFromByteString :: Ptr a -> ByteString -> Int -> IO ()
copyFromByteString ptr bs lenExpected =
  BS.useAsCStringLen bs $ \(cstr, lenActual) -> if lenActual >= lenExpected
...
seedFromBytes bs | BS.length bs < fromIntegral crypto_vrf_seedbytes =
  error $ "Not enough bytes for seed, " <> show @Int (fromIntegral crypto_vrf_seedbytes)
seedFromBytes bs = unsafePerformIO $ do
  seed <- mkSeed
  withForeignPtr (unSeed seed)
    $ \ptr -> copyFromByteString ptr bs (fromIntegral crypto_vrf_seedbytes)
  return seed
...
unsafeRawSeed (Seed fp) = withForeignPtr fp
  $ \ptr -> BS.packCStringLen (castPtr ptr, fromIntegral crypto_vrf_seedbytes)
...
mkVerKey :: IO VerKey
mkVerKey =
  fmap VerKey $ newForeignPtr finalizerFree =<< mallocBytes verKeySizeVRF

-- | Allocate a Signing Key and attach a finalizer. The allocated memory will
-- not be initialized.
mkSignKey :: IO SignKey
mkSignKey =
  fmap SignKey $ newForeignPtr finalizerFree =<< mallocBytes signKeySizeVRF
...
proofFromBytes bs
  | BS.length bs /= certSizeVRF = error "Invalid proof length"
  | otherwise = unsafePerformIO $ do
    proof <- mkProof
    withForeignPtr (unProof proof)
      $ \ptr -> copyFromByteString ptr bs certSizeVRF
    return proof
...
skFromBytes bs = unsafePerformIO $ do
  if bsLen /= signKeySizeVRF
    then do
      ...
      sk <- mkSignKey
      withForeignPtr (unSignKey sk)
        $ \ptr -> copyFromByteString ptr bs signKeySizeVRF
      return sk
...
vkFromBytes bs = unsafePerformIO $ do
  if bsLen /= verKeySizeVRF
    then do
      ...
      pk <- mkVerKey
      withForeignPtr (unVerKey pk)
        $ \ptr -> copyFromByteString ptr bs verKeySizeVRF
      return pk
  where bsLen = BS.length bs
...
-- not be initialized.
mkOutput :: IO Output
mkOutput = fmap Output $ newForeignPtr finalizerFree =<< mallocBytes
  (fromIntegral crypto_vrf_outputbytes)
...
skToVerKey sk = unsafePerformIO $ withForeignPtr (unSignKey sk) $ \skPtr -> do
  pk <- mkVerKey
  withForeignPtr (unVerKey pk) $ \pkPtr -> do
...
skToSeed sk = unsafePerformIO $ withForeignPtr (unSignKey sk) $ \skPtr -> do
  seed <- mkSeed
...
prove sk msg = unsafePerformIO $ withForeignPtr (unSignKey sk) $ \skPtr -> do
  proof <- mkProof
...
verify pk proof msg =
  unsafePerformIO $ withForeignPtr (unVerKey pk) $ \pkPtr -> do
    withForeignPtr (unProof proof) $ \proofPtr -> do
      output <- mkOutput
      BS.useAsCStringLen msg $ \(m, mlen) -> do
        withForeignPtr (unOutput output) $ \outputPtr -> do
          crypto_vrf_verify outputPtr pkPtr proofPtr m (fromIntegral mlen)
            >>= \case
                  0 -> return $ Just $! output
                  _ -> return Nothing

outputFromProof :: Proof -> Maybe Output
outputFromProof (Proof p) = unsafePerformIO $ withForeignPtr p $ \ptr -> do
  output <- mkOutput
  withForeignPtr (unOutput output) $ \outputPtr -> do
...
assertLength l bs | BS.length bs == l = Just bs
                  | otherwise         = Nothing
